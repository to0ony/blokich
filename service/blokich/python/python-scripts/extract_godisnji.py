...
def extract_godisnji_odmor_data(pdf_path):
    vozaci = []
...
if __name__ == "__main__":
    try:
        if len(sys.argv) < 2:
            print("Nedostaje putanja do PDF-a.", file=sys.stderr)
            sys.exit(1)

        pdf_path = sys.argv[1]
        data = extract_godisnji_odmor_data(pdf_path)

        # Spremi JSON u privremenu datoteku
        with tempfile.NamedTemporaryFile(mode="w+", suffix=".json", delete=False, encoding="utf-8") as tmp_file:
            json.dump(data, tmp_file, ensure_ascii=False, indent=2)
            print(tmp_file.name)  # ispiši putanju datoteke
